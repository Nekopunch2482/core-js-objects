...
function compareObjects(obj1, obj2) {
  const f = (v) => Object.entries(v).map(JSON.stringify).sort().join('');

  return f(obj1) === f(obj2);
}
...
function sortCitiesArray(arr) {
  arr.sort(({ city }, { city: city2 }) => city.localeCompare(city2));
  arr.sort(({ country }, { country: country2 }) =>
    country.localeCompare(country2)
  );
  return arr;
}
...
function group(array, keySelector, valueSelector) {
  const res = array.reduce((acc, next) => {
    if (!acc[keySelector(next)]) acc[keySelector(next)] = [];
    acc[keySelector(next)].push(valueSelector(next));
    return acc;
  }, {});
